...
## 6. 重大需求登记：硬件实时监督与监考（2026-08-06）

> 本节登记用户提出的重大新需求，作为架构规划与后续文档增量的入口。详细架构规划见 [hardware-live-guidance-plan.md](./hardware-live-guidance-plan.md)。

### 6.1 需求概览

| 编号 | 需求 | 产品形态 |
|---|---|---|
| R1 | 进军硬件交互 | 智能摄像头/边缘 AI 盒子（软硬解耦） |
| R2 | 实时监督现场指导（教学核心） | AI 实时伴学：旁边看着、实时指出错误 |
| R3 | 监考模式 | 画室月考/模拟考 AI 替代老师监考 |
| R4 | 校考初试在线监考 | 大规模线上考试实时监督（数万学生并发） |
| R5 | 联网数据训练 + 阶段分析 | 学情分析 + 同届基准，判断是否落后/所处阶段 |

### 6.2 影响结论

- **性质**：增量扩展（新增独立服务 + 独立数据域），不重构现有「上传 → 3s 诊断」链路。
- **硬性约束**：① 数据库 schema 变更窗口需冻结；② 合规（人脸/未成年人/大规模监控）须编码前评审通过。
- **并行策略**：现有主线可并行推进，新增能力以独立模块承载。
...
|---|---|---|
| M0 | 四契约文档冻结 + 合规评审 | P0 |
| M1 | 软件化实时伴学（移动/Web 摄像头 + WebRTC + 实时指导） | P0 |
| M2 | 单考场监考 + 学情分析（阶段/同届基准） | P1 |
| M3 | 大规模在线监考（SFU 集群 + 边缘节点 + 异步录制） | P2 |
| M4 | 硬件商业化（硬件抽象层 + 边缘盒子 + 订阅计费） | P2/P3 |

### 6.4 非目标（本期不做）

- 自训视觉基础大模型（本期用数据采集 + 同届基准替代）
- 毫秒级实时推理（3s 指导延迟满足美术纠错体验）
- 自有硬件大规模量产（先软件化验证）
