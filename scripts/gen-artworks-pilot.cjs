 ...
 * 素材库 100 张试点图片生成器
 * ----------------------------
 * 读取 public/data/artworks.json 前 100 条，按 category/style/era 生成确定性 PNG。
 * 输出两层结构：
...
const PILOT_COUNT = 100;

const data = JSON.parse(fs.readFileSync(DATA_FILE, 'utf8'));
const items = (data.items || []).slice(0, PILOT_COUNT);
...
for (const item of items) {
  ...
  console.log(`[${item.id}] ${item.category}/${item.style}  ${item.title}  thumb=${thumbKB}KB(${thumbMs}ms) full=${fullKB}KB(${fullMs}ms)`);
}
...
const estimateMB = Math.round((totalThumbKB + totalFullKB) * (9999 / items.length) / 1024);
const estimateMin9999 = Math.round((totalMs * (9999 / items.length)) / 1000 / 60 * 10) / 10;

console.log('\n===== 试点统计 =====');
console.log(`生成数量: ${items.length} 张缩略图 + ${items.length} 张全图`);
...
console.log(`试点合计: ${Math.round((totalThumbKB + totalFullKB) / 1024 * 10) / 10} MB, 总耗时 ${totalMs} ms`);
console.log(`按 9999 条估算全量: ${estimateMB} MB, 约 ${estimateMin9999} 分钟`);
